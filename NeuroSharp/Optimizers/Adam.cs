...
namespace NeuroSharp.Optimizers
{
    public class Adam
    {
        ...
        public void Step(ParameterizedLayer layer, int t, double eta = 0.001f, double beta1 = 0.9f, double beta2 = 0.999f, double epsilon = 0.0000001f, bool includeBias = true)
        {
            ...
            {
                _meanWeightGradient[i] = beta1 * _meanWeightGradient[i] + (1 - beta1) * layer.WeightGradients[i];
                _varianceWeightGradient[i] = beta2 * _varianceWeightGradient[i] + (1 - beta2) * layer.WeightGradients[i].PointwisePower(2);
